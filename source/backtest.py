...
class Backtest:
    ...
    def run(self):
        ...
        if (self.DISPLAY_RESULTS_GRAPH): self.setupResultsHeader()
        ...
        for i in range(1, len(df)):
            ...
            if (self.DISPLAY_RESULTS_GRAPH): self.addToResults()

        #id self.DISPLAY_RESULTS_GRAPH is true, display results graph
        if (self.DISPLAY_RESULTS_GRAPH): self.displayGraph()
        #else print results statement to console
        remaining_assets = self.CASH + (self.QUANTITY * self.DATA["CURRENT_CLOSE"])
        total_trades = self.SUCCESSFUL_TRADES + self.FAILED_TRADES
        success_percentage = round( ( (self.SUCCESSFUL_TRADES / total_trades) * 100 ), 2)
        ...
        print("Successful Trades: " + str(self.SUCCESSFUL_TRADES) + " Failed Trades: " + str(self.FAILED_TRADES))
    ...
    def orderSuccess(self, quantity, BUYorSELL, GAINorLOSS):
        data = ""
        if (BUYorSELL): 
            data = '{} | Bought {} of {} at {} - Average Share Price: {}'.format(self.DATA["CURRENT_DATE"], quantity, self.SYMBOL, self.DATA["CURRENT_CLOSE"], self.AVERAGE_SHARE_PRICE)
            self.BUY_JUST_OCCURRED = True
        else: 
            data = '{} | Sold {} of {} at {} - Average Share Price: {}'.format(self.DATA["CURRENT_DATE"], quantity, self.SYMBOL, self.DATA["CURRENT_CLOSE"], self.AVERAGE_SHARE_PRICE)
            if (GAINorLOSS): data += ' --> GAIN'
            else: data += ' --> LOSS'
    ...
    def buy(self, quantity):
        ...
        self.AVERAGE_SHARE_PRICE = round( (self.TOTAL_FUNDS_INVESTED / self.QUANTITY), 2)
    ...
    def sell(self, quantity):
        ...
        if (self.DATA["CURRENT_CLOSE"] >= self.AVERAGE_SHARE_PRICE): 
            ...
            successOrFail = True
        else: self.FAILED_TRADES += 1

        #process sell
        funds_returned = quantity * self.DATA["CURRENT_CLOSE"]
        self.QUANTITY -= quantity
        if (self.QUANTITY == 0): self.TOTAL_FUNDS_INVESTED = 0
        else: self.TOTAL_FUNDS_INVESTED = (self.QUANTITY * self.AVERAGE_SHARE_PRICE)
        if (self.QUANTITY != 0): self.AVERAGE_SHARE_PRICE = round( (self.TOTAL_FUNDS_INVESTED / self.QUANTITY), 2)
    ...
    def fundsAvailable(self, quantity):
        funds_required = quantity * self.DATA["CURRENT_CLOSE"]
        if (funds_required <= self.CASH): return True
        return False
    ...
    def setupResultsHeader(self):
        headerMessage = 'Date,{}.Open,{}.High,{}.Low,{}.Close,Assets,BUY,SELL'.format(self.SYMBOL,self.SYMBOL,self.SYMBOL,self.SYMBOL)
        io.writeToFile(self.PATH_TO_RESULTS_FILE, headerMessage)

    #appends single line to backtest results file
    def addToResults(self):
        currentAssets = round( (self.CASH + (self.QUANTITY * self.DATA["CURRENT_CLOSE"])), 2)
        buyValue = "---"
        sellValue = "---"
        if (self.BUY_JUST_OCCURRED): buyValue = self.DATA["CURRENT_CLOSE"]
        if (self.SELL_JUST_OCCURRED): sellValue = self.DATA["CURRENT_CLOSE"]
        next_entry = '{},{},{},{},{},{},{},{}'.format(
    ...
    def displayGraph(self):
        ...
        if (self.USE_CANDLESTICK_ON_RESULTS_GRAPH):
            ...
            fig.append_trace(go.Scatter(x=df['Date'], y=df[(self.SYMBOL + '.Close')], text="Price", name="Price", line=dict(color='rgb(0, 0, 0)')), row=1, col=1)

        #append buy and sell signals
        #.....
        fig.append_trace(go.Scatter(x=df['Date'], y=df['BUY'], mode='markers', text="BUY", name="BUY", line=dict(color='rgb(0, 128, 0)')), row=1, col=1)
        fig.append_trace(go.Scatter(x=df['Date'], y=df['SELL'], mode='markers', text="SELL", name="SELL", line=dict(color='rgb(255, 0, 0)')), row=1, col=1)

        #create total assets over time graph
        fig.append_trace(go.Scatter(x=df['Date'], y=df["Assets"], text="Assets", name="Assets", line=dict(color='rgb(0, 0, 0)')), row=2, col=1)
